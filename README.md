...
3. 抽到炸弹猫时，若有拆除卡会自动消耗并把炸弹放回牌堆；没有拆除卡则立即出局。
...
- 跳过 / 超级跳过：结束当前回合抽牌，或直接结束剩余全部回合。
- 预见未来 / 改变未来：查看并操控牌堆顶部顺序。
- 抽底 / 顶底互换 / 洗牌：改变抽牌位置或整体牌序。
...
- Python 3.9+
- Windows（当前 GUI 使用 ctypes.windll 控制控制台窗口显示）
...
这是一个单机对战版本，适合快速体验炸弹猫核心机制；如需扩展新卡牌，可直接在 Card 子类体系中新增类型并在牌堆初始化中注册数量。
